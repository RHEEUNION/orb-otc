...
import { BABYJUB_SUBORDER, bigintTo32Be, bigintTo32Le, bytesToBigintLE } from '@orbinum/protocol';
import { buildShieldNote, decryptMemo, fastMulBase, packPoint, toHex, viewTag } from './note.mjs';
import { fastMulPoint, unpackUsableViewingKey } from '@orbinum/protocol';
...
const b = toHex(bigintTo32Le(ivkPacked));
...
// recipient-side scan check, as the Hub does: shared = ivsk * ephPk, then compare the view tag in nonce[0]
const m = Uint8Array.from(n.memo.slice(2).match(/../g).map((h) => parseInt(h, 16)));
const shared = bigintTo32Le(fastMulPoint(unpackUsableViewingKey(bytesToBigintLE(m.slice(148))), ivsk)[0]);
console.log('view tag ok:', m[0] === viewTag(shared));
